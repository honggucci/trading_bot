...
def build_fib_ratio_boundaries(df, res, *,
    ...
    def _nearest_key(d, x):  # d는 dict (rets 또는 exts)
        return min(d.keys(), key=lambda kk: abs(float(kk) - float(x)))
    ...
    def level_for_ratio(r):
        r = float(r)
        if r <= 1.0:
            rk = r if r in rets else _nearest_key(rets, r)
            return float(rets[rk])
        else:
            if exts:
                rk = r if r in exts else _nearest_key(exts, r)
                return float(exts[rk])
            # 안전용: 확장 dict 없으면 간단히 스윙에서 외삽
            sw = res['swing']
            if sw['direction'] == 'up':
                lo, hi = sw['start_price'], sw['end_price']
                return float(hi + (hi - lo) * r)
            else:
    ...
    bounds = []
    for r in ratios:
        lvl  = level_for_ratio(r)
        # 밴드 폭은 ATR 기반 — 확장도 동일 로직 사용(필요시 k_ext 별도 도입 가능)
        half = float(k * g(float(min(r, 1.0))) * atr_end)  # r>1.0일 때 과도팽창 방지
        if max_half_mult is not None:
            half = min(half, float(max_half_mult * atr_end))
        bounds.append({
            'ratio': float(r),
            'label': f'R{int(round(float(r)*100))}',
            'level': lvl,
            'low':   lvl - half,
            'high':  lvl + half,
            'half':  half
...
def make_level_map_from_res(res, ratios):
    ...
    for r in ratios:
        if r == 0.0:
            ...
            price = float(res['retracements'].get(r))
...
# finalize_last : 마지막 스윙을 강제로 확정(리포트에 최신 고점/저점 반영하기 좋음)
    ...
    res = fib_from_latest(df, pivot_col='pivot', pivot_price_col='pivot_price', include_extremes=True)
    ...
    with redirect_stdout(buf):
        print_top_down_reports(
            df, df_, res,
            top_tag=top_time, down_tag=down_time,                 # 라벨 표기
            rsi_period_daily=14, rsi_period_4h=14,
            fib_ratios=(1.0, 0.786, 0.618, 0.5, 0.382, 0.236, 0.0),  # R100/R0 포함
            fib_k=1.0, fib_mode='linear', fib_max_half_mult=1.2,
            use_low_4h=False
        )
